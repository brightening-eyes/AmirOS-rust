// slabmalloc's allocation for our kernel
...
impl Pager
{
const BASE_PAGE_SIZE: usize = memory::PAGE_SIZE;
const LARGE_PAGE_SIZE: usize = memory::PAGE_SIZE_2M;

/// Allocates a given `page_size`.
fn alloc_page(&mut self, page_size: usize) -> Option<*mut u8>
{
let r = memory::kernel_alloc(Layout::from_size_align(page_size, page_size).unwrap());
...
memory::kernel_dealloc(VirtAddr::from_mut_ptr_of(ptr), layout);
}

/// Allocates a new ObjectPage from the System.
fn allocate_page(&mut self) -> Option<&'static mut ObjectPage<'static>>
{
self.alloc_page(Pager::BASE_PAGE_SIZE).map(|r| unsafe { transmute(r as usize) })
}
...
{
self.alloc_page(Pager::LARGE_PAGE_SIZE).map(|r| unsafe { transmute(r as usize) })
}
...
{
self.dealloc_page(p as *const LargeObjectPage as *mut u8, Pager::LARGE_PAGE_SIZE);
}
}

/// A pager for GlobalAlloc.
static PAGER: Mutex<Pager> = Mutex::new(Pager);
...
// to avoid fragmentation
PAGER.lock().allocate_large_page().expect("Can't allocate page?") as *mut _ as *mut u8
}
...
{
unsafe { PAGER.lock().allocate_page().map_or(ptr::null_mut(), |page| {
...
unsafe { PAGER.lock().allocate_large_page().map_or(ptr::null_mut(), |large_page| {
